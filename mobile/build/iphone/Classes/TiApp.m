...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kTiSocialACSAppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kTiSocialACSAppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
