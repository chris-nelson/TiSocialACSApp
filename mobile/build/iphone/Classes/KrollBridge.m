...
NSString * TiSocialACSApp$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation TiSocialACSAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_tisocialacsapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create TiSocialACSApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "TiSocialACSApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_tisocialacsapp = [[TiSocialACSAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_tisocialacsapp];
	
	NSString *_tisocialacsappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _tisocialacsappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_tisocialacsapp valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:TiSocialACSApp$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.tisocialacsapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.tisocialacsapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
